...
const PreVentaModule = () => {
  ...
  const formatFechaParaJava = (date) => {
    ...
    // Construye el formato YYYY-MM-DDTHH:mm:ss sin la "Z" de UTC
    return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}`;
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    // Las validaciones de DatePicker ya previenen esto visualmente,
    // pero mantenemos esta doble verificación por seguridad.
    ...
    if (preVenta.fechaCierre && preVenta.fechaEntrega && preVenta.fechaCierre >= preVenta.fechaEntrega) {
      setAlertModal({ show: true, title: "Error de Fechas", message: "La fecha de cierre debe ser antes del día de entrega." });
      return;
    }
  ...
  const inputStyle = "w-full bg-[#f8f9f5] dark:bg-flavis-dark border border-[#326371]/10 dark:border-white/10 p-4 rounded-2xl outline-none focus:border-flavis-gold transition-all text-flavis-blue dark:text-white font-bold text-sm cursor-pointer font-sans placeholder-flavis-blue/30 dark:placeholder-white/10";

  // Obtener la fecha actual una sola vez para consistencia
  const hoy = new Date();

  return (
    <div className="animate-in pb-20 font-sans">
      <h2 className="text-4xl font-main font-bold text-flavis-blue dark:text-white italic mb-10 text-center sm:text-left transition-colors">Configuración de Pre-Venta</h2>
      ...
      <div className="mb-10 max-w-xs">
        <div className="bg-white dark:bg-flavis-card-dark p-8 rounded-[2.5rem] shadow-sm border border-flavis-blue/5 dark:border-white/5 transition-colors">
          ...
          <p className="text-5xl font-black text-flavis-gold tracking-tighter">
            {Math.max(0, preVenta.stockMaximo - totalVendidas)}
          </p>
          <p className="text-[9px] font-bold text-flavis-blue/30 dark:text-white/20 mt-2">Límite configurado: {preVenta.stockMaximo} galletas</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-flavis-card-dark p-6 sm:p-10 rounded-[3rem] shadow-xl border border-[#326371]/5 dark:border-white/5 max-w-4xl mx-auto lg:mx-0 transition-colors">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* --- BLOQUE DE CALENDARIOS CON VALIDACIÓN CRUZADA DINÁMICA --- */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="custom-datepicker">
              ...
              <DatePicker
                ...
                minDate={hoy} // No permite fechas pasadas
                maxDate={preVenta.fechaCierre} // No puede ser después del cierre
                showTimeSelect
                timeFormat="HH:mm"
                timeIntervals={15}
                timeCaption="Hora"
            ...
            <div className="custom-datepicker">
              ...
              <DatePicker
                ...
                minDate={preVenta.fechaApertura || hoy} // Debe ser después de la apertura
                maxDate={preVenta.fechaEntrega} // No puede ser después de la entrega
                showTimeSelect
                timeFormat="HH:mm"
                timeIntervals={15}
                timeCaption="Hora"
            ...
            <div className="custom-datepicker">
              ...
              <DatePicker
                ...
                minDate={preVenta.fechaCierre || preVenta.fechaApertura || hoy} // Debe ser después del cierre
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-[#326371]/5 dark:border-white/5 pt-8">
            <div>
              <label className="block text-[10px] uppercase font-bold text-flavis-blue dark:text-white/40 mb-2 ml-2 tracking-widest opacity-60 font-sans">Límite Total de Galletas</label>
              <input required type="number" className={inputStyle}
                value={preVenta.stockMaximo} onChange={e => setPreVenta({...preVenta, stockMaximo: e.target.value})} />
            </div>
          </div>
