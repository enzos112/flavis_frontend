...
const CookieCard = ({ cookie, quantity, onUpdate, onOpenModal, isPack = false }) => {
  return (
    ...
    >
      {/* INDICADOR DE PACK (Badge) */}
      {isPack && (
        <div className="absolute top-3 left-3 z-10 bg-flavis-gold text-flavis-blue text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg">
          Pack x4 📦
        </div>
      )}
      ...
      <div className="p-4">
        <h3 className="text-flavis-gold font-main text-lg mb-2 leading-tight uppercase tracking-tight">
          {cookie.nombre}
        </h3>
        ...
        <div className="flex justify-between items-center font-secondary">
          ...
          <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
            ...
            > − </button>
            <span className="text-white font-bold text-sm w-4 text-center text-with-symbols">
              {quantity}
            </span>
            <button 
              onClick={() => onUpdate(1)}
